...
@RestController
@RequestMapping("/announcementFiles/*")
@Slf4j
...
//    @PostMapping("upload")
//    @ResponseBody
//    public List<String> uploadA (@RequestParam("file") List<MultipartFile> multipartFiles) throws IOException {
//        List<String> uuids = new ArrayList<>();
//        String path = "C:/upload/Announcement/" + getPath();
//        log.info("path는 " + path);
...
//    }
        ...
        public Map<String, Object> suggestUpload(@RequestParam("file") List<MultipartFile> multipartFiles) throws IOException {
            ...
            String path = ABSOLUTE_PATH + "/Announcement/" + getPath();
            String filePath = "";
            File file = new File(path);
            if (!file.exists()) {
                file.mkdirs();
            ...
            for (int i = 0; i < multipartFiles.size(); i++) {
                ...
                if (multipartFiles.get(i).getContentType().startsWith("image")) {
                    ...
                    InputStream inputStream = new FileInputStream("C:\\upload\\Announcement\\" + getPath() + "\\" + uuids.get(i)+ "_" + multipartFiles.get(i).getOriginalFilename());
