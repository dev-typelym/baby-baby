...
function openModal(num, e) {
    var selectedModal = '#review-detail' + num;
...
const $contentWrap = $(".review-board-list");
const $detailContentWrap = $(".review-modal-list");
...
$('.category-entire').click(function() {
    adminReviewSearch.categoryType = null;
    adminReviewSearch.reviewContent = null;

    getAdminReviewBoardList();
...
$('.category-country-side').click(function() {
    adminReviewSearch.categoryType = 'AGRICULTURE';
    getAdminReviewBoardList();
...
$('.category-gallery-art').click(function() {
    adminReviewSearch.categoryType = 'ART';
    getAdminReviewBoardList();
...
$('.category-tradition').click(function() {
    adminReviewSearch.categoryType = 'TRADITION';
    getAdminReviewBoardList();
...
$('.category-craft').click(function() {
    adminReviewSearch.categoryType = 'CRAFT';
    getAdminReviewBoardList();
...
$('.category-science').click(function() {
    adminReviewSearch.categoryType = 'SCIENCE';
    getAdminReviewBoardList();
...
$('.category-museum').click(function() {
    adminReviewSearch.categoryType = 'MUSEUM';
    getAdminReviewBoardList();
...
$('.category-sports').click(function() {
    adminReviewSearch.categoryType = 'SPORTS';
    getAdminReviewBoardList();
...
$('.category-etc').click(function() {
    adminReviewSearch.categoryType = 'ETC';
    getAdminReviewBoardList();
...
function getAdminReviewBoardList() {
    $.ajax({
        url: `reviewList/${globalThis.page}`,
        data: adminReviewSearch,
...
$(".search-btn-icon").on("click", function (e) {
    ...
    adminReviewSearch.reviewContent = val;

    console.log( adminReviewSearch.reviewContent + "777");
    getAdminReviewBoardList();
    $(".search-input").val("");
});
...
function showList(reviewBoardDTOS) {
    var content = "";
    var detailContent = "";
    reviewBoardDTOS.forEach(reviewBoard => {

        const formattedDate = formatDate(new Date(reviewBoard.writeDate));
        const formattedEventDate = formatDate(new Date(reviewBoard.startDate));
        let changeCategoryName = null;
        if (reviewBoard.eventCategory === null) {
            changeCategoryName = '전체';
        } else if (reviewBoard.eventCategory === 'AGRICULTURE'){
            changeCategoryName = '농촌';
        } else if (reviewBoard.eventCategory === 'ART'){
            changeCategoryName = '예술';
        } else if (reviewBoard.eventCategory === 'TRADITION'){
            changeCategoryName = '전통';
        } else if (reviewBoard.eventCategory === 'CRAFT'){
            changeCategoryName = '공방';
        } else if (reviewBoard.eventCategory === 'SCIENCE'){
            changeCategoryName = '과학';
        } else if (reviewBoard.eventCategory === 'MUSEUM'){
            changeCategoryName = '박물관';
        } else if (reviewBoard.eventCategory === 'SPORTS') {
        ...
        content +=
            `
               <tr class="row" onclick="openModal(${reviewBoard.id}, event)">
                    <td class="no-modal">
                        <input type="checkbox" name="check">
                    </td>
                    <td class="review-id">${reviewBoard.id}</td>
                    <td>${changeCategoryName}</td>
                    <td>${reviewBoard.eventTitle}</td>
                    <td>${reviewBoard.nickName}</td>
                    <td>${reviewBoard.eventRecruitCount}</td>
                    <td>${formattedEventDate}</td>
                    <td>${formattedDate}</td>
                    <td>${reviewBoard.reviewScore}</td>
                    <td>${reviewBoard.reviewContent}</td>
                </tr>
            `

        detailContent +=
            `
                <section class="user-modal" id="review-detail${reviewBoard.id}">
                    ...
                    onclick="closeModal(${reviewBoard.id})"
                    >
                    ...
                    <h5 class="detail-title">후기 상세정보</h5>
                    <div class="profile-img-wrapper">
                    <label>
                    <div class="content-img">
                        ${
                reviewBoard.reviewFileDTOS && reviewBoard.reviewFileDTOS.length > 0
                    ? reviewBoard.reviewFileDTOS.map(fileDTO => `<img src="/announcementFiles/display?fileName=Review/${fileDTO.filePath}/${fileDTO.fileUUID}_${fileDTO.fileOriginalName}" />`).join('')
                    ...
                    <p>${reviewBoard.eventTitle}</p>
                    </div>
                    </li>
                    <li class="content-list">
                    <span>작성자</span>
                    <div class="content-input">
                    <p>${reviewBoard.nickName}</p>
                    </div>
                    </li>
                    <li class="content-list">
                    <span>후기 제목</span>
                    <div class="content-input">
                    <p>${reviewBoard.reviewTitle}</p>
                    </div>
                    </li>
                    <li class="content-list">
                    <span>참여일시</span>
                    ...
                    <span>별점</span>
                    <div class="content-input">
                    <p>${reviewBoard.reviewScore}</p>
                    </div>
                    </li>
                    <li class="content-list">
                    <span>내용</span>
                    <div class="content-input content-div">
                    <p>${reviewBoard.reviewContent}</p>
                    </div>
                    </li>
                    </ul>
                    </main>
...
$('.confirm-delete').on('click', function () {
    ...
    // 체크 박스 체크된 값
    $('input:checkbox[name=check]:checked').closest('tr').find('.review-id').each(function () {
