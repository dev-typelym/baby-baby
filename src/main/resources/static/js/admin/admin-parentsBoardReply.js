...
const $contentWrap = $(".parents-yard-board-reply-list");
const adminParentsBoardReplySearch = {
    parentsBoardReplyContent: null
};
...
function getAdminParentsBoardReplyList() {
    $.ajax({
        url: `parentsBoardReplyList/${globalThis.page}`,
        data: adminParentsBoardReplySearch,
...
$(".search-btn-icon").on("click", function (e) {
    ...
    adminParentsBoardReplySearch.parentsBoardReplyContent = val;

    console.log( adminReviewSearch.parentsBoardReplyContent + "777");
    getAdminParentsBoardReplyList();
    $(".search-input").val("");
});
